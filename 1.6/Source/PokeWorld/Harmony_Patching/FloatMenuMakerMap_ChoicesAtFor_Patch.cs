...
public class FloatMenuOptionProvider_Pokemon : FloatMenuOptionProvider
{
    ...
    {
        var list = new List<FloatMenuOption>();
        var comp = clickedPawn.TryGetComp<CompPokemon>();
        if (comp == null)
        {
            return list;
        }
        if (clickedPawn.MentalStateDef != null || clickedPawn.playerSettings == null || clickedPawn.playerSettings.Master == null || !clickedPawn.playerSettings.Master.Drafted)
        {
            return list;
        }
        var clickedCell = context.ClickedCell;
        if (!clickedCell.InBounds(clickedPawn.Map)) return list;
        if (clickedPawn.Map != Find.CurrentMap) return list;
        if (clickedCell.Fogged(clickedPawn.Map))
        {
            var floatMenuOption = GotoLocationOption(clickedCell, clickedPawn);
            if (floatMenuOption == null)
            {
                return list;
            }
        ...
        AddDraftedOrders(context.clickPosition, clickedPawn, list);
        foreach (var item in clickedPawn.GetExtraFloatMenuOptionsFor(clickedCell)) list.Add(item);
        return list;
    }
    ...
    {
        var clickCell = IntVec3.FromVector3(clickPos);
        foreach (var item in GenUI.TargetsAt(clickPos, TargetingParameters.ForAttackAny(), true))
        {
            if (!item.HasThing || !item.Thing.HostileTo(Faction.OfPlayer)) continue;
            var attackTarg = item;
            var comp = pawn.TryGetComp<CompPokemon>();
            if (comp != null && comp.moveTracker != null && PokemonAttackGizmoUtility.CanUseAnyRangedVerb(pawn))
            {
